...
def escolher_e_plotar_multiplos_colunas(dataframes, x_col_index, y_col_index, plot_choice, graph_title):
    ...
    for i, df in enumerate(dataframes):
        ...
        group_index = i // 2
        color = cores[group_index]

        if i % 2 == 0:
            label = f'Teste {group_index + 1} (agrupado do teste {i+1} e {i+2})'
            if y_col == 'N/mm2' and x_col == '%':
                max_test_value = max_values[-1][1]
                max_test_x_value = max_values[-1][2]
                label = f'Teste {group_index + 1} (Ponto máximo: {max_test_value:.2f} N/mm² em {max_test_x_value:.2f}%)'
        else:
            label = None
    ...
    # Encontrar o valor máximo de deformação em todos os testes
    max_deformacao = max([x[-1] for x in x_values])

    # Definir common_x até o valor máximo de deformação
    common_x = np.linspace(0, max_deformacao, num=500)
    ...
    if plot_choice in ['media', 'media_desvio', 'media_desvio_all']:
        ...
        plt.plot(common_x, y_values_mean, linestyle='--', color='black', linewidth=1, label='Média')
        ...
        if save_path:
            media_df = pd.DataFrame({'Deformação (%)': common_x, 'Tensão Média (N/mm²)': y_values_mean})
    ...
    plt.title(graph_title if graph_title else f'{y_col_legend} vs {x_col_legend}')
    plt.yticks(np.arange(0, 15, step=1))  # Define o eixo y com intervalos de 1 a 1
    plt.xticks(np.arange(0, 1100, step=100))  # Define o eixo x com intervalos de 100 a 100
    
    # Ajustar para que o gráfico comece em (0,0)
    plt.xlim(left=0)
    plt.ylim(bottom=0)
    ...
    max_values_str = '\n'.join([f'Teste {mv[0]}: Máximo = {mv[1]:.2f} N/mm² em {mv[2]:.2f} %' for mv in max_values])
