...
def escolher_e_plotar_multiplos_colunas(dataframes, x_col_index, y_col_index, plot_choice, graph_title):
    ...
    for i, df in enumerate(dataframes):
        ...
        if plot_choice in ['all', 'media_desvio_all']:
            plt.plot(x, y, linestyle='-', linewidth=1, label=f'Teste {i+1} (Ponto Máximo: {max_value:.2f} N em {max_x_value:.2f} mm)')
    ...
    max_values_str = '\n'.join([f'Teste {mv[0]}: Máximo = {mv[1]:.2f} N em {mv[2]:.2f} mm' for mv in max_values])
