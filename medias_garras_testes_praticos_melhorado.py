...
def ler_multiplos_csv(caminhos_arquivos):
    ...
    for caminho in caminhos_arquivos:
        try:
            df = pd.read_csv(caminho, delimiter=',', decimal='.', skipinitialspace=True, encoding='utf-8')
        except UnicodeDecodeError:
            # Tente uma codificação alternativa
            df = pd.read_csv(caminho, delimiter=',', decimal='.', skipinitialspace=True, encoding='latin1')
...
def escolher_e_plotar_multiplos_colunas(dataframes, x_col_index, y_col_index, plot_choice, graph_title):
    ...
    # Definindo os nomes dos testes
    nomes_testes = ['Beak', 'Constant Curve', 'Straight']
    ...
    for i, df in enumerate(dataframes):
        ...
        # Considerar valores negativos (não em módulo)
        x = df[x_col].values
        y = df[y_col].values
        ...
        if plot_choice in ['all', 'media_desvio_all']:
            plt.plot(x, y, linestyle='-', linewidth=1, label=f'{nomes_testes[i]} (Ponto Máximo: {max_value:.2f} N em {max_x_value:.2f} mm)')
    ...
    plt.ylim(-2, 12)  # Fixar a escala do eixo Y entre -2 e 12

    # Ajustar espaçamento do eixo X para 10 em 10 mm e alinhar os valores no eixo Y
    plt.xticks(np.arange(0, max_x_global * 1.05, step=10), ha='center')

    # Ajustar espaçamento do eixo Y para 1 em 1 N
    plt.yticks(np.arange(-2, 13, step=1))  # Ajusta a escala do eixo Y de -2 até 12 com incremento de 1

    # Adicionar uma linha horizontal em y = 0 para cruzar com o eixo X
    ...
    if plot_choice in ['desvio', 'media_desvio', 'media_desvio_all']:
        ...
        # Atualizar as legendas dos eixos e título do gráfico
    plt.xlabel("Deslocamento (mm)")  # Eixo X com Deslocamento em mm
    plt.ylabel("Força (N)")  # Eixo Y com Força em N
    
    # Ajuste para o título não ser cortado
    plt.title(graph_title if graph_title else f'Força (N) vs Deslocamento (mm)', pad=20)
    
    plt.legend(loc='upper right')
    plt.grid(True)

    # Função para ajustar as margens e garantir que a legenda não seja cortada
    plt.tight_layout()  # Ajusta automaticamente as margens para que nada seja cortado
    
    plt.show()

    # Exibir valores máximos em uma janela
    max_values_str = '\n'.join([f'{nomes_testes[mv[0]-1]}: Máximo = {mv[1]:.2f} N em {mv[2]:.2f} mm' for mv in max_values])
