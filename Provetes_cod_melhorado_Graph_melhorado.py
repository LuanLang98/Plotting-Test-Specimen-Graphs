...
def escolher_e_plotar_multiplos_colunas(dataframes, x_col_index, y_col_index, plot_choice, graph_title):
    ...
    for i, df in enumerate(dataframes):
        ...
        if plot_choice in ['all', 'media_desvio_all']:
            plt.plot(x, y, linestyle='-', linewidth=1, label=f'Teste {i+1} (Ponto Máximo: {max_value:.2f} N/mm² em {max_x_value:.2f} %)')
    ...
    plt.ylim(0, max_y_global * 1.05)  # Adicione um pequeno espaço extra no eixo Y

    # Ajustar espaçamento do eixo X para 100 em 100
    plt.xticks(np.arange(0, max_x_global * 1.05, step=100))

    # Ajustar espaçamento do eixo Y para 1 em 1
    plt.yticks(np.arange(0, max_y_global * 1.05, step=1))
